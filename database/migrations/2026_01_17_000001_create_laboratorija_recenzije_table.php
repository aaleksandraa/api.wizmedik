<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        Schema::create('laboratorija_recenzije', function (Blueprint $table) {
            $table->id();
            $table->foreignId('laboratorija_id')->constrained('laboratorije')->onDelete('cascade');
            $table->foreignId('user_id')->nullable()->constrained()->onDelete('set null');
            $table->string('ime')->nullable(); // Za gost korisnike
            $table->tinyInteger('ocjena')->unsigned();
            $table->text('komentar')->nullable();
            $table->boolean('verifikovano')->default(false);
            $table->boolean('odobreno')->default(false);
            $table->string('ip_adresa')->nullable();
            $table->timestamps();

            $table->index(['laboratorija_id', 'odobreno']);
            $table->index('created_at');
        });
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('laboratorija_recenzije');
    }
};
