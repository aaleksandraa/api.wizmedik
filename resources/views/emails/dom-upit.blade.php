...
<body>
    <div class="header">
        ...
        <p style="margin: 10px 0 0 0;">{{ $dom->naziv }}</p>
    </div>
    ...
    <div class="content">
        ...
        @if($upit->opis_potreba)
        <div class="message-box">
            <span class="label">📋 Opis potreba:</span>
            <p class="value" style="margin: 10px 0 0 0; white-space: pre-line;">{{ $upit->opis_potreba }}</p>
        </div>
        @endif

        @if($upit->zelja_posjeta)
        <div class="info-row">
            <span class="label">📅 Želja za posjetom:</span>
            <span class="value">{{ \Carbon\Carbon::parse($upit->zelja_posjeta)->format('d.m.Y') }}</span>
        </div>
        @endif
